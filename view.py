...
async def process_callback_start(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ {callback_query.from_user.username}! Начать игру ❓ ⬅", reply_markup=keyboard)
...
async def selecting_total_quantity(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(chat_id=callback_query.message.chat.id, text="➡Выберите общее количество конфет 🍩 ⬅", reply_markup=keyboard)
...
async def random_total_number(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id, text=f"{callback_query.from_user.username}! Выбрал {total_candies} 🍩")
    await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ Общее количество 🍩 на столе 🟰 {total_candies} ⬅")
    ...
    await bot.send_message(chat_id=callback_query.message.chat.id, text="➡  Выберите максимальную количество 🍩 за один ход ⬅", reply_markup=keyboard)
...
async def random_max_number(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id, text=f"{callback_query.from_user.username} выбрал {max_candies} 🍩")
    await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ Максимальное  количество 🍩 за один ход, которые могут быть взяты 🟰 {max_candies} ⬅")
...
async def random_first_move(callback_query: types.CallbackQuery):
    ...
    if first_move == 1:
        ...
        await bot.answer_callback_query(callback_query.id, text=f"➡ {callback_query.from_user.username}! Ты берешь 🍩 первым!⬅")
        ...
        await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ Ты берешь 🍩 первым! Выберите число от 1 до {max_candies} ⬅", reply_markup=keyboard)
        
    else:
        await bot.answer_callback_query(callback_query.id, text=f"{callback_query.from_user.username}! 🐵 берет первая!")
        await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ {callback_query.from_user.username}! 🐵 берет первая ⬅")
        
        bot_move = total_candies % (max_candies + 1) if total_candies % (max_candies + 1) != 0 else random.randint(1, (max_candies + 1))
        total_candies -= bot_move
        
        if total_candies == 0:
            await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ 🐵 взяла последний 🍩! 🐵 выиграла игру! ⬅")
            return
          
        await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ 🐵 взяла {bot_move} 🍩. На столе осталось {total_candies} 🍩. Ваша очередь, выберите число от 1 до {max_candies}⬅")
        ...
        await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ Ваша очередь, выберите число от 1 до {max_candies} ⬅", reply_markup=keyboard)
...
async def playing_with_bot(callback_query: types.CallbackQuery):
    ...
    if selected_number > max_candies:
        await bot.answer_callback_query(callback_query.id, text=f"➡ {callback_query.from_user.username}. Выбранное число слишком велико. Повторите попытку, возьмите от 1 до {max_candies} ⬅")
        
    else:
        total_candies -= selected_number
        
        if total_candies == 0:
            await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ {callback_query.from_user.username}, ты взял последний 🍩! Ты выиграл игру! ⬅")
            return
          
        await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ {callback_query.from_user.username}, ты взял {selected_number} 🍩. Осталось {total_candies} 🍩 . Очередь 🐵! ⬅")
        
        bot_move = total_candies % (max_candies + 1) if total_candies % (max_candies + 1) != 0 else random.randint(1, (max_candies + 1))
        total_candies -= bot_move
        
        if total_candies == 0:
            await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ 🐵 забрал последний 🍩! 🐵 выиграла игру! ⬅")
            return
          
        await bot.send_message(chat_id=callback_query.message.chat.id, text=f"➡ {callback_query.from_user.username}, 🐵 взяла {bot_move} 🍩. Осталось {total_candies} 🍩 . Ваша очередь, выберите число от 1 до {max_candies} ⬅")
